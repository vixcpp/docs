...
import { highlightCpp, highlightShell, highlightText, normalizeLang } from "./highlighter";
...
html:not(.dark) .cb-body { background: #0d1117; }
...
.cb .cb-dir     { color: #c586c0; font-weight: 600; }

/* Inclusion : <iostream>, "vix.hpp" */
.cb .cb-inc     { color: #ce9178; }

/* Contrôle de flux : if, else, for, return, throw, try, catch */
.cb .cb-ctrl    { color: #c586c0; font-weight: 600; }

/* Autres mots-clés : class, struct, const, static, template */
.cb .cb-kw      { color: #569cd6; font-weight: 600; }

/* Types : string, vector, optional, App, Request, ThreadPool */
.cb .cb-type    { color: #4ec9b0; font-weight: 600; }

/* Namespaces : std, vix, chrono */
.cb .cb-ns      { color: #4fc1ff; }

/* Appels de fonction : app.get(...), res.json(...) */
.cb .cb-fn      { color: #dcdcaa; }

/* Builtins : cout, move, make_unique, push_back */
.cb .cb-blt     { color: #dcdcaa; font-style: italic; }

/* Membres : .name, ->value */
.cb .cb-mem     { color: #9cdcfe; }

/* Identifiants standards */
.cb .cb-id      { color: #f8fafc; }

/* Constantes SCREAMING_CASE */
.cb .cb-const   { color: #4fc1ff; font-weight: 600; }

/* Chaînes de caractères */
.cb .cb-str     { color: #ce9178; }

/* Caractères : 'x', '\n' */
.cb .cb-char    { color: #d7ba7d; }

/* Nombres */
.cb .cb-num     { color: #b5cea8; font-weight: 500; }

/* Commentaires */
.cb .cb-cmt     { color: #6a9955; font-style: italic; }

/* Opérateurs : ::, +, =, <, > */
.cb .cb-op      { color: rgba(230, 237, 243, .50); }

/* Flèche -> */
.cb .cb-arrow   { color: rgba(230, 237, 243, .65); }

/* Accolades { } */
.cb .cb-brace   { color: #ffd700; }

/* Parenthèses ( ) */
.cb .cb-paren   { color: rgba(230, 237, 243, .55); }

/* Crochets [ ] */
.cb .cb-bracket { color: #da70d6; }

/* Point-virgule ; */
.cb .cb-semi    { color: rgba(230, 237, 243, .82); }

/* URLs dans le code */
.cb .cb-url     { color: #4fc1ff; text-decoration: underline; text-underline-offset: 2px; }

/* ════════════════════════════════════════════════
   SHELL TOKENS
   ════════════════════════════════════════════════ */
.cb .cb-sh-prompt  { color: #22c55e; font-weight: 800; }
.cb .cb-sh-cmd     { color: #38bdf8; font-weight: 700; }
.cb .cb-sh-flag    { color: #fb923c; }
.cb .cb-sh-path    { color: #a5b4fc; }
.cb .cb-sh-url     { color: #38bdf8; text-decoration: underline; text-underline-offset: 2px; }
.cb .cb-sh-port    { color: #b5cea8; }
.cb .cb-sh-http    { color: #dcdcaa; font-weight: 700; }
.cb .cb-sh-comment { color: #6a9955; font-style: italic; }
