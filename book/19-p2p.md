...
| Peer connection | Connect to another node |
| Handshake | Establish peer identity and session |
| Envelope | Wrap messages with metadata |
| Framing | Split byte streams into complete messages |
| Dispatch | Decode payloads into typed messages |
| Sync messages | Push WAL, acknowledge, pull outbox items |
...
| Best for | Browser real-time | Distributed systems | APIs |
| Discovery | Manual | Auto (UDP) or registry | Manual |
...
[Next: Production deployment](/book/20-production-deployment)
