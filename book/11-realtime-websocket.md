...
In a `vix.app` project, create a WebSocket module from the CLI.
...
Use `--name` when the module name should be provided as an option instead of a positional argument.

```bash
vix modules add --websocket --name live_chat --workflow attached
```

The workflow chooses the generated runtime shape.

| Workflow     | Runtime behavior                              |
| ------------ | --------------------------------------------- |
| `attached`   | HTTP and WebSocket run together.              |
| `standalone` | The module owns a WebSocket server runtime.   |
| `bridge`     | Application setup is bridged to WebSocket.    |
| `client`     | No runtime entry point; support code only.    |

Backend projects generated by Vix include a static WebSocket status panel on the home page. It connects from the browser and shows whether the WebSocket runtime is reachable.
